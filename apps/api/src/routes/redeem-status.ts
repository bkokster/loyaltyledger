...
export async function registerRedeemStatusRoutes(app: FastifyInstance) {
  app.get<{ Params: { redemption_id: string } }>(
    '/v1/redeem/:redemption_id/status',
    async (request, reply) => {
      ...
      const redemptionId = request.params.redemption_id;
      if (!isUuid(redemptionId)) {
        reply.code(400).send({ error: 'Invalid redemption id' });
        return;
      }
      ...
      if (result.rowCount === 0) {
        reply.code(404).send({ error: 'Redemption job not found' });
        return;
      }
