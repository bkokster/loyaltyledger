...
export async function run() {
    ...
    try {
        ...
        reports.forEach((report) => {
            ...
            console.log('[settlement] report generated', JSON.stringify({ tenantId, merchantAccount, netPoints: netPoints.toString(), periodStart, periodEnd }));
        });
    }
    finally {
        await closePool();
...
async function generateSettlementReports() {
    ...
    await withTransaction(async (client) => {
        const result = await client.query(`SELECT j.tenant_id, l.account_id AS merchant_account, SUM(l.cr) - SUM(l.dr) AS net_points
         FROM ledger_lines l
         JOIN ledger_journal j ON j.entry_id = l.entry_id
        WHERE j.ts >= $1 AND j.ts < $2
          AND l.account_id LIKE $3
        GROUP BY j.tenant_id, l.account_id`, [periodStart.toISOString(), periodEnd.toISOString(), '%::merchant_liability']);
...
async function upsertSettlementReport(client, report) {
    await client.query(`INSERT INTO settlement_reports (
